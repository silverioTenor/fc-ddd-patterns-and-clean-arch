...
describe('OrderItem unit tests', () => {
   it('should create an order item', () => {
      const orderItem = new OrderItem(uuid(), uuid(), 'P1', 4, 12);
   ...
   it('should throw an error when creating an order item with invalid ID', () => {
      expect(() => new OrderItem('', uuid(), 'P1', 4, 12)).toThrow('ID is required!');
   });

   it('should throw an error when creating an order item with invalid product ID', () => {
      expect(() => new OrderItem(uuid(), '', 'P1', 4, 12)).toThrow('Product ID is required!');
   });

   it('should throw an error when creating an order item with invalid product name', () => {
      expect(() => new OrderItem(uuid(), uuid(), '', 4, 12)).toThrow('Product name is required!');
   });

   it('should throw an error when creating an order item with invalid quantity', () => {
      expect(() => new OrderItem(uuid(), uuid(), 'P1', 0, 12)).toThrow(
         'Quantity must be greater than zero!',
      );
   });

   it('should throw an error when creating an order item with invalid price', () => {
      expect(() => new OrderItem(uuid(), uuid(), 'P1', 4, 0)).toThrow('Price is required!');
   });
